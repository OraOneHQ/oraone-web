...
Drop a **Universal AI Agent** — chat **and** voice with shared memory — into any
...
// The agent recognises this person across chat AND voice:
...
| `OraOne.startVoice(options?)` | Continue the same visitor on the voice channel. |
| `OraOne.callVisitor(options?)` | Request an outbound AI call to the visitor. |
