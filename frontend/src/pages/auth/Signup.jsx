...
import { AuthShell, GradientButton, GoogleButton, OrLine } from "@/components/auth/AuthShell";
  ...
  return (
    <AuthShell cardMaxWidth="max-w-lg" cardTestId="signup-form">
      ...
      <form className="mt-8 space-y-5" onSubmit={handleSignUp}>
        ...
        <Field label={<>Company Name <span className="font-normal text-[#94A3B8]">(Optional)</span></>}>
          <IconInput
            icon={Building2}
            type="text"
            value={company}
        ...
        <Checkbox
          ...
          checked={agree}
          onChange={setAgree}
          accentClass="peer-checked:border-[#6366F1] peer-checked:bg-[#6366F1] peer-focus-visible:ring-[#6366F1]/15"
          data-testid="signup-terms"
        >
          I agree to the{" "}
          <Link to="/terms" className="font-semibold text-[#6366F1] hover:underline">Terms of Service</Link>{" "}
          and{" "}
          <Link to="/privacy" className="font-semibold text-[#6366F1] hover:underline">Privacy Policy</Link>
        </Checkbox>
      ...
      <GoogleButton data-testid="auth-google" onClick={() => loginWithProvider("Google", "signup")} />

      <p className="mt-6 text-center text-sm text-[#64748B]">
        Already have an account?{" "}
        <Link to="/login" className="font-semibold text-[#6366F1] hover:underline">Sign in</Link>
      </p>
    </AuthShell>
  );
}
