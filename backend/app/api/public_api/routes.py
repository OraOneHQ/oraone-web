...
from app.services import analytics_service, api_key_service, usage_service, webhook_service
    ...
    if agent is not None:
        ...
        if is_new:
            await webhook_service.dispatch(
                ...
                {"conversation_id": conversation_id, "agent_id": str(agent.id), "channel": "chat"},
            )
        await webhook_service.dispatch(
            key.organization_id,
            WebhookEventType.MESSAGE_CREATED,
            {"conversation_id": conversation_id, "message": payload.message[:500]},
        )
