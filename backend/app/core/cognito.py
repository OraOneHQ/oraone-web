...
# Backwards compatibility - use lazy loader
cognito_client = property(lambda self: get_cognito_client()).__get__(None, None)

# Export for direct imports (will lazy-load on first call)
class _CognitoClientProxy:
    def __getattr__(self, name):
        return getattr(get_cognito_client(), name)

cognito_client = _CognitoClientProxy()
