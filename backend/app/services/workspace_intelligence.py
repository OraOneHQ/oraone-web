"""Workspace-intelligence service (org-scoped, Product 2 dashboard).
...
async def optimization_score(session: AsyncSession, org_id: uuid.UUID) -> dict[str, Any]:
    ...
    avg_dur = await _scalar(session, select(func.avg(Conversation.duration_seconds)).where(
        Conversation.organization_id == org_id, Conversation.started_at >= d30,
        cast(Conversation.channel, String) == "voice"), 0.0)
    ...
    voice = _clamp(100 - max(0.0, (avg_dur - 180) / 6)) if avg_dur else 60
    ...
    weights = {
        "knowledge": 0.16, "prompt": 0.12, "voice": 0.10, "latency": 0.10,
        "conversion": 0.14, "security": 0.10, "integrations": 0.08,
        "workflow": 0.08, "analytics": 0.06, "cost": 0.06,
    }
    parts = {
        "knowledge": knowledge, "prompt": prompt, "voice": voice, "latency": latency,
    ...
    if avg_dur and avg_dur > 240:
        recs.append({"area": "Voice", "severity": "low", "title": "Trim call length",
                     "detail": f"Average call is {int(avg_dur)}s. Sharper prompts shorten handle time and cut cost."})
    if not recs:
