...
async def infrastructure(session: AsyncSession) -> dict[str, Any]:
    ...
    services = [
        ...
        svc("PostgreSQL (RDS)", "database", "operational" if db_ok else "down"),
        svc("Redis", "cache"),
...
# --------------------------------------------------------------------------- #
_DEFAULT_FLAGS = [
    ("voice_agents", "Voice Agents", "Enable the voice AI calling platform."),
...
_SECRET_CATALOG: list[tuple[str, str, str]] = [
    ...
    ("DEEPGRAM_API_KEY", "ai", "Deepgram (STT) Key"),
    ("ELEVENLABS_API_KEY", "ai", "ElevenLabs (TTS) Key"),
    ...
    ("COGNITO_CLIENT_ID", "auth", "Cognito App Client ID"),
    ("COGNITO_USER_POOL_ID", "auth", "Cognito User Pool ID"),
    ...
    ("PINECONE_API_KEY", "vector", "Pinecone API Key"),
