...
VisitorProfile when they later "call" (voice keyed by phone), exercising the
alias lookup + identity merge. Creates rows in the test org then cleans up.
"""
import asyncio
import os
import uuid
...
async def main() -> None:
    ...
    async with session_scope() as db:
        try:
            ...
            # 3) Next day they CALL — voice keyed by the normalised phone.
            p_voice = await vs.upsert_profile(
                db, organization_id=ORG,
                visitor_key=vs.normalize_phone(phone), channel="voice",
                phone=phone,
            )
            created.append(p_voice.id)

            same = p_voice.id == p_chat.id
            digest = vs.build_memory_digest(p_voice, current_channel="voice")
            print("chat profile id :", p_chat.id)
            print("voice profile id:", p_voice.id)
            print("SAME identity   :", same)
            print("channels_used   :", p_voice.channels_used)
            print("aliases         :", p_voice.aliases)
            print("--- voice memory digest ---")
            print(digest)
