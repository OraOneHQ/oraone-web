...
> **Gotcha:** if your OS/shell already has `OPENAI_API_KEY` (or any other var
> below) set globally, `python-dotenv` used to leave it alone by default,
> silently ignoring `backend/.env`. Every `load_dotenv(...)` call in this repo
> now passes `override=True` so `backend/.env` always wins — but if you
> vendor this pattern elsewhere, remember to do the same.

### Embeddings (optional — RAG / document + website-crawl ingestion)

| Variable | Default | Notes |
|----------|---------|-------|
| `EMBEDDING_PROVIDER` | `hash` if unconfigured, else auto-detects AWS creds | `hash` (dependency-free, lexical-overlap vectors, always works) or `bedrock` (Amazon Titan v2, needs a valid `AWS_BEARER_TOKEN_BEDROCK`/AWS creds with model access). |
...
## Channels (Twilio — optional, WhatsApp/SMS)

| Variable | Notes |
|----------|-------|
| `TWILIO_ACCOUNT_SID` / `TWILIO_AUTH_TOKEN` | Enable the WhatsApp/SMS channel adapter (`app/services/channels/`). Unset = channel stays disabled, no error. |
| `TWILIO_PHONE_NUMBER` | Sending number for outbound SMS/WhatsApp. |
